...
class Callee extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      messageText: global.locale == 'en' ? ' Calling...' : '呼び出し...',
      usernameText: 'asdsad',
      callStatus: 0,
      videoOn: false,
      callerVideoOn: false,
  ...
  componentDidMount() {
    //twilioVideo.current.connect({ accessToken: global.callerData.accesstoken });

    let self = this;

    global.socket.on('twilio_caller_drop', function (ret) {
      ...
      self.props.navigationRef.current?.navigate('GL');
    });

    global.socket.on('twilio_caller_video_on', function (ret) {
      self.setState({
        calleeVideoOn: true,
      });
    });

    global.socket.on('twilio_caller_video_off', function (ret) {
      self.setState({
        calleeVideoOn: false,
      });
    });
  ...
  answerCall() {
    ...
    global.socket.emit('twilio_callee_answer', global.callerData);
  }
  ...
  videoOn() {
    if (this.state.videoOn) {
      this.setState(
        ...
        () => {
          global.socket.emit('twilio_callee_video_off', global.callerData);
        },
      );
    } else {
      this.setState(
        ...
        () => {
          global.socket.emit('twilio_callee_video_on', global.callerData);
        },
      );
    }
  }
  ...
  back() {
    ...
    global.socket.emit('twilio_callee_drop', global.callerData);
  }
            ...
            <Image
              source={{
                uri:
                  ...
                  global.callerData.profile_photo,
              }}
              style={{
                width: 50,
                height: 50,
